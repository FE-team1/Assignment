...
function Q3() {
  ...
  */
  ...
  // 줄넘기 시작을 클릭하면  isShow의 상태를 true로 변경 => Q3 컴포넌트 랜더링
  const start = () => {
    setIsShow(true);
  };

  // 줄넘기 중지를 클릭하면 isShow의 상태를 false로 변경 => Q3 컴포넌트 랜더링 중지
  const stop = () => {
    setIsShow(false);
  };

  // isShow가 true이면 2초마다 count 값을 1씩 증가시키고 false이면 count의 값을 0으로 초기화한다.
  useEffect(() => {
    if (isShow === true) {
